...
apiurl = "https://api.flickr.com/services/rest/?method=flickr.photos.getRecent&api_key=e399afd03ac0bfada1e71cf10273c4e1&per_page=500&format=json&nojsoncallback=1";
...
img.onload = function () {
	...
	localStorage.setItem(myresult_size.source, canvas.toDataURL("image\png"));
	var image = new Image();
	...
	$("#results").append('<li><a href="'+myresult_size.url+'" target="_blank" class = "thumbnail" onclick = "return false;"><img id = "'+ id++ +'" src="'+myresult_size.source+'" draggable="true" ondragstart = "drag(this)" onclick = "enableFilters(); getFilters(this)"/></a></li>');
}
...
function drawImage(inputImage)
	{
		...
		collage[darthVaderGroup._id] = {
			original: img,
			base64: localStorage.getItem(inputImage.src),
...
function setPhotoSettings(settings) {
	...
	var img = new Image();
	img.src = collage[group._id].base64;
...
function disableSettings() {
	...
	form.degrees.disabled = true;
	form.z.disabled = true;
	form.sepia.disabled = true;
	form.desaturation.disabled = true;
	form.contrast.disabled = true;
...
function enableSettings() {
	...
	form.degrees.disabled = false;
	form.z.disabled = false;
	form.sepia.disabled = false;
	form.desaturation.disabled = false;
...
function send () {
	...
	for (var i in collage)
	{
		...
		var collageWithoutBase64 = {
			...
			z: c.z,
			sepia: c.sepia,
			desaturation: c.desaturation,
			contrast: c.contrast
		};
		var collagejson = JSON.stringify(collageWithoutBase64);
